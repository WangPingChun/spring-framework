...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			// 给 BeanPostProcessors 一个机会来返回代理来替代真正的实例 AOP 的功能就是基于这个地方
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			/*
			 * 创建 bean 实例，并将实例包裹在 BeanWrapper 实现类对象中返回。包含三种创建 bean 实例的方式：
			 * 1、通过工厂方法创建 bean 实例
			 * 2、通过构造方法自动注入（autowire by constructor）的方式创建 bean 实例
			 * 3、通过无参构造方法创建 bean 实例
			 *
			 * 若 bean 的配置信息中配置了 lookup-method 和 replace-method，则会使用 CGLIB 增强 bean
			 * 实例
			 */
			instanceWrapper = createBeanInstance(beanName, mbd, args);
		}
		// 包装的实例对象，此处的 bean 可以认为是一个原始的 bean 实例，暂未填充属性
		...
		if (earlySingletonExposure) {
			...
			// earlySingletonReference 只有在检测到有循环依赖情况下才不会为空
			if (earlySingletonReference != null) {
				...
				// 处理依赖
					 ...
					 * actualDependentBeans 不为空则表示当前 bean 创建后其依赖的 bean 却没有全部创建完，也就是
					 * 存在循环依赖
					 */
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					// 调用 InstantiationAwareBeanPostProcessor : 实例化前置处理器
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		/*
		 * 1、ctors != null -> 后置处理器返回构造方法数组是否为空
		 * 2、mbd.getResolvedAutowireMode() == AUTOWIRE_CONSTRUCTOR -> bean 配置中的 autowire 属性是否
		 * 配置为 constructor
		 * 3、mbd.hasConstructorArgumentValues() -> constructorArgumentValues 是否存在元素，即 bean 配置
		 * 文件中是否配置了 <construct-arg />
		 * 4、!ObjectUtils.isEmpty(args) -> args 数组是否存在元素
		 *
		 * 上面4个条件，只要又一个为 true，就会通过构造方法自动注入的方式构造 bean 实例
		 */
	...
	protected BeanWrapper obtainFromSupplier(Supplier<?> instanceSupplier, String beanName) {
		...
		// 设置新的 bean 的对象名到 currentlyCreatedBean 中
		...
		try {
			...
			// 设置原创建的 bean 的对象名到 currentlyCreatedBean 中
			if (outerBean != null) {
		...
		try {
			...
			if (System.getSecurityManager() != null) {
				beanInstance = AccessController.doPrivileged(
						// 获得 InstantiationStrategy 对象，并使用它创建 bean 对象
	...
	protected BeanWrapper autowireConstructor(
		...
		// 创建 ConstructorResolver 对象，并调用 autowireConstructor 方法
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic()
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					// 返回值为是否继续填充 bean， postProcessAfterInstantiation: 如果应该在 bean 上面设置属性则返回 true，
					// 否则返回 false 一般情况下，应该是返回 true，如果返回 false 的话，将会阻止在此 bean 实例上调用任何后续的
					// InstantiationAwareBeanPostProcessors 实例
		...
		if (resolvedAutowireMode == AUTOWIRE_BY_NAME || resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
			...
			// MutablePropertyValues 允许对属性封装进行简单的操作，并提供构造函数以支持 Map 的深度复制和构造
	...
	protected void autowireByName(
		...
		/*
		 * 获取非简单类型属性的名称，且该属性未被配置在配置文件中，Spring 认为的"简单类型"属性有：
		 ...
		 * 8、以上所有类型的数组形式
		 *
		 * 除了要求非见简单类型的属性外，还要求属性未在配置文件中配置过，也就是 pvs.contains(pd.getName()) = false
		 */
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					// 获取 setter 方法（write method）的参数信息，比如参数在参数列表中的位置，参数类型，以及该参数
					// 所归属的方法等信息
					...
					// 解析指定 beanName 属性所匹配的值，并把解析的属性名称存储在 autowiredBeanNames 中，当属性存在过个封装 bean 时
					// 将会找到所有匹配的 bean 并将其注入
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		for (PropertyDescriptor pd : pds) {
			if (pd.getWriteMethod() != null // 有可写方法
					&& !isExcludedFromDependencyCheck(pd) // 依赖检查中没有被忽略
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		// 获取对应的解析器，用于将 bean 定义对象中包含的值解析为应用于目标 bean 实例的实际值
	...
	private Object convertForProperty(
		...
		// 若 TypeConverter 为 BeanWrapperImpl 类型，则使用 BeanWrapperImpl 来进行类型转换，这里主要是因为 BeanWrapperImpl 实现
		// 了 PropertyEditorRegistry 接口
