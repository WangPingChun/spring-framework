...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 即它不会被考虑作为其他 bean 自动装配的候选者，但是该 bean 本身还是可以使用自动装配来注入
	 * 其他 bean 的。对应 bean 属性 autowire-candidate
	 */
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		// 获取类中对应方法名的个数，当方法重载时，count 的值就会大于 1
		...
		if (count == 0) {
			...
			// 标记 overloaded 暂未被覆盖，在使用 CGLIB 增强阶段不需要进行校验，避免参数类型检查的开销
