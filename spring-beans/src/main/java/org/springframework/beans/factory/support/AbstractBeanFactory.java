...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 * GenericBeanDefinition 转换成 RootBeanDefinition
	 */
	...
	protected <T> T doGetBean(
		...
		// note: 单例模式的 bean 在整个过程中只会被创建一次,第一次创建后会将该 bean 加载到缓存中,后续在获取 bean 就会直接从单例缓存中获取
		...
		if (sharedInstance != null && args == null) {
			...
			// note: 缓存中记录的是最原始的 bean 状态,我们得到的不一定是我们最终想要的 bean
			...
			/*
			   note:
			   1.对于单例模式,Spring 在创建 bean 的时候并不是等 bean 完全创建完成后才会将 bean 添加到缓存中,而是不等 bean 创建完成就会将
			   创建 bean 的 ObjectFactory 提早加入到缓存中,这样一旦下一个 bean 创建的时候需要依赖 bean 时则直接使用 ObjectFactory
			   2.对于原型模式,我们知道是没办法使用缓存的,所以 Spring 对于原型模式的循环依赖处理策略是不处理
			 */
			...
			// parentBeanFactory 不为空 && beanDefinitionMap 中不存在该 beanName 的 BeanDefinition 实例
			if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
				...
				// 明确的 args 从 parentBeanFactory 中获取 bean 对象
				else if (args != null) {
					// Delegation to parent with explicit args.
					return (T) parentBeanFactory.getBean(nameToLookup, args);
				}
				// 明确的 requiredType 从 parentFactory 中获取 bean 对象
				else if (requiredType != null) {
					// No args -> delegate to standard getBean method.
					return parentBeanFactory.getBean(nameToLookup, requiredType);
				}
				// 直接使用 beanName 从 parentFactory 中获取 bean 对象
				else {
					return (T) parentBeanFactory.getBean(nameToLookup);
				}
			}

			// 如果不是仅仅做类型检查则是创建 bean 这里需要做记录
			if (!typeCheckOnly) {
			...
			try {
				...
				// 从容器中获取 beanName 相应的 GenericBeanDefinition 对象,并将其转换为 RootBeanDefinition
				RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				// 检查给定的合并的 BeanDefinition
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				// 处理所依赖的 bean note: 对于依赖的 bean 他会优先加载,所以在 Spring 的加载顺序中,在初始化某一个 bean 的时候,首先会初始化这个 bean 的依赖
				String[] dependsOn = mbd.getDependsOn();
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						// 若给定的依赖 bean 已经注册为依赖给定的 bean 循环依赖的情况
				...
				if (mbd.isSingleton()) {
					...
					sharedInstance = getSingleton(beanName, () -> {
						try {
							...
							// 显式从单例缓存中删除 bean 实例,因为单例模式下为了解决循环依赖,可能他已经存在了,所以销毁他
		...
		// 如果传递的 requiredType 不为 null，则需要检测所得到的 bean 的类型是否与该 requiredType 一致。如果不一致则尝试转换，当然也
		// 是要能够转换成功，否则抛出异常
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		...
		// 获取 RootBeanDefinition 如果返回的 BeanDefinition 是子类 bean 的话则合并父类相关属性
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			// 若 beanInstance 不是 FactoryBean 类型则抛出异常
			if (!(beanInstance instanceof FactoryBean)) {
			...
			// name 以 "&" 开头 并且 beanInstance 为 FactoryBean 则直接返回 FactoryBean 不需要通过 getObject 获取其返回
			return beanInstance;
		}
		...
		if (mbd != null) {
			...
			// 若 BeanDefinition 为空,则从缓存中加载 bean 对象
			object = getCachedObjectForFactoryBean(beanName);
		}
		// 若 object 依然为空,则可以确认 beanInstance 一定是 FactoryBean 从而使用 FactoryBean 获取 bean 对象
		if (object == null) {
			...
			if (mbd == null && containsBeanDefinition(beanName)) {
				// 将存储 XML 配置文件的 GenericBeanDefinition 转换为 RootBeanDefinition 如果指定 beanName 是子 bean 的话
				// 同时会合并父类的相关属性
	 ...
	 *                 bean 的名字
	 * @param mbd      the merged bean definition for the bean
	 *                 已经合并了父类属性的（如果有的话）的 BeanDefinition 对象
