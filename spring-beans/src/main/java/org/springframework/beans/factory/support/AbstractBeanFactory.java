...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
			...
			throws BeansException {
		// 提取对应的 beanName(这里的 name 不一定就是 beanName，可能是 aliasName，也可能是 FactoryBean)
			...
			try {
				...
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						...
						// beanA 要求 beanB 在其之前被创建，但是 beanB 又要求 beanA 先于它创建。这个
						// 时候形成了循环，对于 depends-on 循环，Spring 会直接抛出异常
				...
				if (mbd.isSingleton()) {
					sharedInstance = getSingleton(beanName, () -> {
						try {
							...
							// 显示从单例缓存中删除 bean 实例，因为单例模式下为了解决循环依赖，可能它已经存在了，所以销毁它
	...
	protected String transformedBeanName(String name) {
		...
		return canonicalName(
				// 去除 FactoryBean 的修饰符，也就是如果 name="&aa"，name会首先去除 & 而使 name="aa"
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		// Quick check on the concurrent map first, with minimal locking.缓存中获取
	...
	protected RootBeanDefinition getMergedBeanDefinition(
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				// bd.getParentName() == null，表示无父配置，这时直接将当前的 BeanDefinition 升级为 RootBeanDefinition
				if (bd.getParentName() == null) {
					...
					try {
						...
						/*
						 * 判断父类 beanName 和子类 beanName 名称是否相同。若相同，则父类 bean 一定在父容器中。
						 * 原因也很简单，容器底层使用 Map 缓存键值对的。
						 */
	...
	protected Object getObjectForBeanInstance(
		...
		// 验证：如果指定的 name 是工厂相关（以&为前缀）且 beanInstance 又不是 FactoryBean 类型则验证不通过
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			// 如果是 NullBean，则直接返回
			if (beanInstance instanceof NullBean) {
				return beanInstance;
			}
			// 如果 beanInstance 不是 FactoryBean 类型，则抛出异常（以'&'开头的必定是 FactoryBean 类型）
			if (!(beanInstance instanceof FactoryBean)) {
		...
		Object object = null;
		if (mbd == null) {
			// 尝试从 cache 中加载 bean，FactoryBean 生成的单例 bean 会被缓存在 factoryBeanObjectCache 集合
			// 中，不用每次都创建
		...
		if (object == null) {
			...
			if (mbd == null && containsBeanDefinition(beanName)) {
				// 将存储 XML 配置文件的 GenericBeanDefinition 转换为 RootBeanDefinition，如果指定 BeanName 是
				// 子 Bean 的话同时会合并父类的相关属性
				mbd = getMergedLocalBeanDefinition(beanName);
			}
			// 是否用户定义的而不是应用程序本身定义的
			// synthetic 字面意思是"合成的"，在 AOP 相关的类中会将该属性设为 true。
