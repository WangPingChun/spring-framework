...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		if (explicitArgs != null) {
			...
			// 缓存中存在,则解析存储在 BeanDefinition 中的参数,如给定方法的构造函数 A(int, int) 则通过此方法后就会把配置文件中的 ("1","1")
			// 转换为 (1,1),缓存中的值可能是原始值也又可能是最终值
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			// 只有一个构造函数 并且 getBean 未传递参数 并且构造函数为无参构造函数
			// 直接创建 bean
			...
			// 对构造函数进行排序处理: public 构造函数优先参数数量降序,非 public 构造函数参数数量降序
			...
			for (Constructor<?> candidate : candidates) {
				...
				// 如果已经找到选用的构造函数或者需要的参数个数小于当前的构造函数参数个数,则终止.因为已经按照参数个数降序排列了
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (factoryBeanName != null) {
			...
			// 工厂名为空,则其可能是一个静态工厂,静态工厂创建 bean 必须要提供工厂的类类名
			if (!mbd.hasBeanClass()) {
				throw new BeanDefinitionStoreException(mbd.getResourceDescription(), beanName,
		...
		// 获取 factoryMethodToUse、argsHolderToUse、argsToUse 属性
		Method factoryMethodToUse = null;    // 工厂方法
		ArgumentsHolder argsHolderToUse = null;
		Object[] argsToUse = null;    // 参数

		// 如果指定了构造参数则直接使用在调用 getBean 方法的时候指定了方法参数
		if (explicitArgs != null) {
			argsToUse = explicitArgs;
		} else {
			Object[] argsToResolve = null;
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			if (candidates == null) {
				...
				for (Method candidate : rawCandidates) {
					// 如果又 static 且为工厂方法,则添加到 candidates 中
					if (Modifier.isStatic(candidate.getModifiers()) == isStatic && mbd.isFactoryMethod(candidate)) {
			...
			for (Method candidate : candidates) {
				...
				if (parameterCount >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						// 根据参数创建参数吃鱼哦这 ArgumentsHolder 对象
						argsHolder = new ArgumentsHolder(explicitArgs);
					} else {
						...
						try {
							...
							// 获取 ParameterNameDiscoverer 对象 ParameterNameDiscoverer 是用于解析方法和构造函数的参数名称的接口，为参数名称探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							// 获取指定构造函数的参数名称
							if (pnd != null) {
								paramNames = pnd.getParameterNames(candidate);
