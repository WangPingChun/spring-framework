...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 校验 BeanDefinition, 这是注册前的最后一次校验了，主要是对 methodOverrides 进行校验
		...
		if (existingDefinition != null) {
			...
			// 覆盖 beanDefinition 大于被覆盖的 beanDefinition 的 ROLE,打印 info 日志
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		// 初始化参数名称发现器,该方法并不会在这个时候尝试检索参数名称 getParameterNameDiscover 返回 ParameterNameDiscoverer 实例
		descriptor.initParameterNameDiscovery(getParameterNameDiscoverer());
		// 依赖类型为 Optional 类型
		if (Optional.class == descriptor.getDependencyType()) {
			return createOptionalDependency(descriptor, requestingBeanName);
		}
		// 依赖类型为 ObjectFactory ObjectProvider
		...
		//     // javaxInjectProviderClass 类注入的特殊处理
			...
			// 为实际依赖关系目标的延迟解析构建代理,默认实现返回 null
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			// 针对给定的工厂给定一个快捷实现的方式,例如考虑一些预先解析的信息,在进入所有 bean 的常规类型匹配算法之前,解析算法将首先尝试通过此
			// 方法解析快捷方式
			...
			// 解析复合 bean 其实就是对 bean 的属性进行解析,包含: 数组、Collection、Map 类型
			Object multipleBeans = resolveMultipleBeans(descriptor, beanName, autowiredBeanNames, typeConverter);
			if (multipleBeans != null) {
				return multipleBeans;
			}

			// 查找与类型相匹配的 bean，返回值构成: key = 匹配的 beanName, value = beanName 对应的实例化 bean
			Map<String, Object> matchingBeans = findAutowireCandidates(beanName, type, descriptor);
			// 没有找到 校验 @Autowired 的 require 是否为 true
			if (matchingBeans.isEmpty()) {
				if (isRequired(descriptor)) {
					// 如果为 true 但是还是没有找到,则抛出异常
			...
			if (matchingBeans.size() > 1) {
				// 确定给定 bean autowire 的候选者,按照 @Primary 和 @Priority 的顺序
