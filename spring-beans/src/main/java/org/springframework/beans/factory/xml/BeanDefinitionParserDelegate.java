...
public class BeanDefinitionParserDelegate {
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			// 解析 lookup-method 属性
			// note: Spring 动态改变 bean 里方法的实现.方法执行返回的对象,使用 Spring 内原有的这类对象替换,通过
			// 改变方法返回值来动态改变方法.内部实现为使用 cglib 方法,重新生成子类,重写配置的方法和返回对象,达到动态
			// 改变的效果.
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			// 解析 replaced-method 属性
			// note: Spring 动态改变 bean 里方法的实现.需要改变的方法,使用 Spring 内原有其他类（需要该类继承集合
			// {@link org.springframework.beans.factory.support.MethodReplacer}）的逻辑,替换这个方法.
			// 通过改变方法逻辑来动态改变方法.
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		// note: 设置当前 bean 在被其他对象作为自动注入对象的时候,是否作为候选 bean, 默认值为 true
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			// <meta key="special-data" value="special" />
	...
	public void parseQualifierElement(Element ele, AbstractBeanDefinition bd) {
		...
		try {
			...
			// 解析 value 属性 设置到 AutowireCandidateQualifier 中
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// 多个元素存在,报错
		if ((hasRefAttribute && hasValueAttribute) ||    // ref 和 value 都存在
				((hasRefAttribute || hasValueAttribute) && subElement != null)) {    // ref 和 value 存在一,并且 subElement 存在
			error(elementName +
					" is only allowed to contain either 'ref' attribute OR 'value' attribute OR sub-element", ele);
		}

		// 将 ref 属性值构建为 RuntimeBeanReference 对象
		if (hasRefAttribute) {
		...
		// 将 value 属性值构造为 TypedStringValue 对象
