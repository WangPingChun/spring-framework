...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	/**
	 * EntityResolver 的作用就是,通过实现它,应用可以自定义如果寻找"验证文件"的逻辑.
	 * <p>
	 * 如果 SAX 应用程序需要实现自定义处理外部实体,则必须实现此接口并使用 setEntityResolver 方法向
	 * SAX 驱动器注册一个实例.也就是说,对于解析一个 XML,SAX 首先读取该 XML 文档上的声明,根据声明去寻找
	 * 相应的 DTD 定义,以便对文档进行一个验证.默认的寻找规则,即通过网络(实现上就是声明 DTD 的 URL 地址)
	 * 来下载相应的 DTD 声明,并进行验证.下载的过程是一个漫长的过程,而且当网络中断或不可用时,这里会报错,就是
	 * 应为相应的 DTD 声明没有被找到的原因.
	 */
	@Nullable
