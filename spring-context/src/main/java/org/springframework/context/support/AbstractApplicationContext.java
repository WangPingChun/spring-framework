...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	/**
	 * Name of the MessageSource bean in factory.
	 * If none is supplied, message resolution is delegated to the parent.
	 *
	 * @see MessageSourcethe
	 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			// 准备刷新的上下文环境：记录下容器的启动时间、标记"已启动"状态、处理配置文件中的占位符
			prepareRefresh();
			...
			// 对 BeanFactory 进行各种功能填充：设置 BeanFactory 的类加载器，添加几个 BeanPostProcessor，手动注册几个特殊的 bean
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 子类覆盖方法做额外的处理：到这里的时候，所有的 Bean 都加载、注册完成了，但是都还没有初始化，具体的子类可以在这步的时候添加
				// 一些特殊的 BeanFactoryPostProcessor 的实现类来做点什么事
				...
				// 初始化应用消息广播器，并放入 applicationEventMulticaster bean
				...
				// 销毁已经初始化的 singletons 的 beans，以免有些 bean 会一直占用资源
				destroyBeans();
	...
	protected void prepareRefresh() {
		...
		// 初始化 context environment 中的占位符属性来源，留给子类覆盖
		initPropertySources();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置 beanFactory 的表达式语言处理器，Spring 3 开始增加了对语言表达式的支持，默认可以使用 #{bean.xxx} 的形式来调用相关属性值
		...
		// 设置忽略自动装配的接口，如果某个 bean 依赖于以下几个接口的实现类，在自动装配的时候忽略它们，Spring 会通过其它方式来处理这些依赖
		...
		// 这个 BeanPostProcessor 在 bean 实例化后，如果是 ApplicationListener 的子类，那么会将其添加到 listener 列表中，可以理解成：
		// 注册事件监听器
	...
	protected void initMessageSource() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 包含 "messageSource" 的 bean
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			this.messageSource = beanFactory.getBean(MESSAGE_SOURCE_BEAN_NAME, MessageSource.class);
			// Make MessageSource aware of parent MessageSource.
			// 如果有父类，HierarchicalMessageSource 分级处理的 MessageSource
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource) {
				HierarchicalMessageSource hms = (HierarchicalMessageSource) this.messageSource;
				if (hms.getParentMessageSource() == null) {
					// Only set parent context as parent MessageSource if no parent MessageSource
					// registered already.
					// 如果没有注册父 MessageSource，则设置为父类上下文的 MessageSource
