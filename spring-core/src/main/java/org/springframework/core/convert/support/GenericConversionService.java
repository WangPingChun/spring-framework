...
public class GenericConversionService implements ConfigurableConversionService {
	...
	public void addConverter(Converter<?, ?> converter) {
		// 获取 ResolvableType 对象基于 converter.getClass() 类
	...
	public Object convert(@Nullable Object source, @Nullable TypeDescriptor sourceType, TypeDescriptor targetType) {
		Assert.notNull(targetType, "Target type to convert to cannot be null");
		// 如果 sourceType 为空则直接处理结果
		if (sourceType == null) {
			Assert.isTrue(source == null, "Source must be [null] if source type == [null]");
			return handleResult(null, targetType, convertNullSource(null, targetType));
		}
		// 如果类型不对则抛出 IllegalArgumentException 异常
	...
	protected GenericConverter getConverter(TypeDescriptor sourceType, TypeDescriptor targetType) {
		...
		// 从 converterCache 缓存中获得 GenericConverter 对象
		...
		// 如果获取不到则从 converters 中查找
		converter = this.converters.find(sourceType, targetType);
		// 如果查找不到则获取默认的 Converter 对象
		if (converter == null) {
			converter = getDefaultConverter(sourceType, targetType);
		}

		// 如果找到 converter 则添加 converter 到 converterCache 并返回
		if (converter != null) {
			this.converterCache.put(key, converter);
			return converter;
		}

		// 如果找不到 converter 则添加 NO_MATCH 占位符到 converterCache 中并返回 null
	...
	private Object handleConverterNotFound(
		...
		// 如果 source 为空则直接返回 null
		if (source == null) {
			assertNotPrimitiveTargetType(sourceType, targetType);
			return null;
		}
		// 如果 sourceType 为空或者 targetType 是 sourceType 的子类，则返回 source
	...
	private static class Converters {
		...
		private GenericConverter getRegisteredConverter(TypeDescriptor sourceType,
			...
			// 从 globalConverters 中获得 globalConverter
