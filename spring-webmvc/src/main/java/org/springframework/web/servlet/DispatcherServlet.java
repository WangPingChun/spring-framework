...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 如果开启探测功能，则扫描已注册的 HandlerMapping 的 Beans，添加到 handlerMappings 中
		if (this.detectAllHandlerMappings) {
			// Find all HandlerMappings in the ApplicationContext, including ancestor contexts.
			// 扫描已注册的 HandlerMapping 的 beans
			Map<String, HandlerMapping> matchingBeans = BeanFactoryUtils.beansOfTypeIncludingAncestors(context, HandlerMapping.class, true, false);
			if (!matchingBeans.isEmpty()) {
				// 添加到 handlerMappings 中并进行排序
	...
	private void initHandlerExceptionResolvers(ApplicationContext context) {
		...
		if (this.detectAllHandlerExceptionResolvers) {
			// 自动扫描 HandlerExceptionResolver 的 beans
			...
			// 获得名字为 HANDLER_EXCEPTION_RESOLVER_BEAN_NAME 的 bean
			try {
		...
		// 如果未获取到，则获取默认配置的 HandlerExceptionResolver 类
	...
	private void initViewResolvers(ApplicationContext context) {
		...
		// 如果未获取到则从获取默认配置的 ViewResolver 类
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		...
		// 根据 入参的className 获取 Properties 当中的 value 值
		...
		if (value != null) {
			...
			// 遍历 classNames 数组，创建对应的类添加到 strategyInterface 中
			for (String className : classNames) {
				try {
					// 获得 class
					Class<?> clazz = ClassUtils.forName(className, DispatcherServlet.class.getClassLoader());
					// 创建 className 对应的类并添加到 strategies 中
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			...
			for (ViewResolver viewResolver : this.viewResolvers) {
				// 根据 viewName 和 locale 参数解析处 View 对象
