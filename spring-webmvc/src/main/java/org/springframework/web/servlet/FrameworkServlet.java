...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 * 标记是否接收到了 ContextRefreshedEvent 事件。即 {@link #onApplicationEvent(ContextRefreshedEvent)}
	 */
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		// 如果构造方法已经传入了 webApplicationContext 属性，则直接使用
		if (this.webApplicationContext != null) {
			// A context instance was injected at construction time -> use it
			// 赋值给 wac 变量
			wac = this.webApplicationContext;
			// 如果是 ConfigurableWebApplicationContext 类型，并且未激活则进行初始化
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 设置 wac 的父 context 未 rootContext 对象
	...
	protected WebApplicationContext findWebApplicationContext() {
		...
		// 从 ServletContext 中获得属性名对应的 WebApplicationContext 对象
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// 执行处理完 WebApplicationContext 后的逻辑
	...
	protected void doTrace(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 如果 dispatchTraceRequest 为 true，则处理该氢气
	...
	private void publishRequestHandledEvent(HttpServletRequest request, HttpServletResponse response,
		...
		if (this.publishEvents && this.webApplicationContext != null) {
			...
			// 创建 ServletRequestHandledEvent 并进行发布
