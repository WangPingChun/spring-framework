...
public abstract class HttpServletBean extends HttpServlet implements EnvironmentCapable, EnvironmentAware {
	...
	/**
	 * 必须配置的属性的集合。在 {@link ServletConfigPropertyValues} 中会校验是否有对应的属性。
	 */
	...
	public final void init() throws ServletException {
		...
		if (!pvs.isEmpty()) {
			try {
				// 将当前的这个 Servlet 对象转化成一个 BeanWrapper 对象。从而能够以 Spring 的方式来将 pvs 注入到该 BeanWrapper 对象中
				...
				// 空实现，留给子类覆盖
				initBeanWrapper(bw);
				// 以 Spring 的方式来将 pvs 注入到 BeanWrapper 对象中
