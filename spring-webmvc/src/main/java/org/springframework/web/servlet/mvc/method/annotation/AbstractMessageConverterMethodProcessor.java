...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	...
	protected <T> void writeWithMessageConverters(@Nullable T value, MethodParameter returnType,
		...
		if (isContentTypePreset) {
			// 如果存在 ContentType 的值并且不包含通配符，则使用它作为 selectMediaType
			...
			// 从请求中，获得可接受的 MediaType 数组，默认实现是从请求头的 Accept 中获取
			List<MediaType> acceptableTypes = getAcceptableMediaTypes(request);
			// 获得可产生的 MediaType 数组
			List<MediaType> producibleTypes = getProducibleMediaTypes(request, valueType, targetType);
			// 如果 body 非空，并且无可产生的 MediaType 数组，则抛出异常
			if (body != null && producibleTypes.isEmpty()) {
				throw new HttpMessageNotWritableException("No converter found for return value of type: " + valueType);
			}
			// 通过 acceptableTypes 来对比，将符合的 producibleTypes 添加到 mediaTypesToUse 结果数组中
			...
			// 如果没有符合的 mediaType 并且 body 非空，则抛出异常
	...
	protected List<MediaType> getProducibleMediaTypes(
		...
		// 先从 PRODUCIBLE_MEDIA_TYPES_ATTRIBUTE 属性中获取，该属性的来源是 @RequestMapping(producer = xxx)
		Set<MediaType> mediaTypes = (Set<MediaType>) request.getAttribute(HandlerMapping.PRODUCIBLE_MEDIA_TYPES_ATTRIBUTE);
		// 如果非空，则使用该属性
		if (!CollectionUtils.isEmpty(mediaTypes)) {
			return new ArrayList<>(mediaTypes);
		} else if (!this.allSupportedMediaTypes.isEmpty()) {
			// 如果 allSupportedMediaTypes 非空，则遍历 messageConverters 数组，进行类型匹配
