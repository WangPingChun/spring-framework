...
public class ExceptionHandlerExceptionResolver extends AbstractHandlerMethodExceptionResolver
	...
	private void initExceptionHandlerAdviceCache() {
		...
		// 扫描 @ControllerAdvice 注解的 bean，并将结果进行排序
		...
		for (ControllerAdviceBean adviceBean : adviceBeans) {
			...
			// 扫描 ControllerAdviceBean 的方法
			ExceptionHandlerMethodResolver resolver = new ExceptionHandlerMethodResolver(beanType);
			// 如果方法有 @ExceptionHandler 注解，则添加到 exceptionHandlerAdviceCache 中
			if (resolver.hasExceptionMappings()) {
				this.exceptionHandlerAdviceCache.put(adviceBean, resolver);
			}
			// 如果该 beanType 是 ResponseBodyAdvice 子类，则添加到 responseBodyAdvice 中
	...
	protected ServletInvocableHandlerMethod getExceptionHandlerMethod(@Nullable HandlerMethod handlerMethod, Exception exception) {
		...
		if (handlerMethod != null) {
			...
			if (method != null) {
				// 如果获得到 Method 方法，则创建 ServletInvocableHandlerMethod 对象并返回
				return new ServletInvocableHandlerMethod(handlerMethod.getBean(), method);
			}
			// For advice applicability check below (involving base packages, assignable types
			// and annotation presence), use target class instead of interface-based proxy.
			// 获得 handlerType 的原始类，因为此处可能是代理对象
