 ...
 * 对于相同的视图名，返回的是相同的 View 对象，所以可以通过缓存进一步提高性能
...
public abstract class AbstractCachingViewResolver extends WebApplicationObjectSupport implements ViewResolver {
	...
	public View resolveViewName(String viewName, Locale locale) throws Exception {
		...
		if (!isCache()) {
			...
			// 如果获得不到缓存，则从 viewCreationCache 中获得 View 对象
			if (view == null) {
				// synchronized 锁
				synchronized (this.viewCreationCache) {
					// 从 viewCreationCache 中获得 View 对象
					...
					if (view == null) {
						...
						// 如果 view 非空，则添加到 viewAccessCache 和 viewCreationCache 缓存中
						if (view != null) {
